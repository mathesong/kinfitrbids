...
### Change when updated ###
regions <- c("Pons;Midbrain")

regions_split <- str_split(regions, ";")[[1]]

tacdata <- bids_parse_files(paramsX$studypath) %>% 
  unnest(filedata) %>% 
  filter(measurement=="tacs") %>% 
  mutate(tacs = map(path_absolute, ~read.delim(.x, sep="\t"))) %>% 
  select(ses:run, tacs, desc) %>% 
  filter(desc == seg) %>% 
  mutate(tacs = map(tacs, ~.x %>% 
                      ...
                      gather(Region, Radioactivity, -frame_start, -frame_end, 
                                                    -t_tac, -dur) %>% 
                      mutate(Radioactivity = unit_convert(Radioactivity, 
                                                          from_units="Bq",
                                                          to_units = "kBq")) %>% 
...
# Fit Model
kb <- as.numeric(ifelse(config$Model$AIF$spline_kb == "", yes = 10, 
             no = config$Model$AIF$spline_kb))
ka_m <- as.numeric(ifelse(config$Model$AIF$spline_ka_m == "", yes = 10, 
             no = config$Model$AIF$spline_ka_m))
ka_a <- as.numeric(ifelse(config$Model$AIF$spline_ka_a == "", yes = 10, 
